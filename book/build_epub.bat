...
set "OUT=C:\git\everand\book\%AUTHOR%_%TITLE%_%LANG%.epub"

echo Python: "%PYTHON%"
"%PYTHON%" --version
"%WKHTML%" --version

echo Spoustim generovani EPUB do: "%OUT%"
...
REM set "DUMP_HTML=C:\git\everand\debug\bookbuilder_pdf.html"
REM "%PYTHON%" C:\git\everand\tools\bookbuilder\bookbuilder.py --input "C:\git\everand\book" --output "%OUT%" --wkhtmltopdf "%WKHTML%" --dump-pdf-html "%DUMP_HTML%" --title "%TITLE%" --author "%AUTHOR%" --lang "%LANG%"
"%PYTHON%" C:\git\everand\tools\bookbuilder\bookbuilder.py --input "C:\git\everand\book" --output "%OUT%" --wkhtmltopdf "%WKHTML%" --title "%TITLE%" --author "%AUTHOR%" --lang "%LANG%"
